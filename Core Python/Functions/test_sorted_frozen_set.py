...
class TestIterableProtocol(unittest.TestCase):
    ...
    def test_for_loop_unpythonic(self):
        expected_index = 0
        for item in self.s:
            self.assertEqual(item, self.expected[expected_index])
            expected_index += 1

    def test_for_loop_pythonic(self):
        for i, j in zip(self.s, self.expected):
...
class TestSequenceProtocol(unittest.TestCase):
    ...
    def test_index_one_beyond_the_end(self):
        # This implementation or the lambda
    ...
    def test_slice_from_start(self):
        self.assertTrue(
            self.s[:3] == SortedFrozenSet([1, 4, 9])
        )
    ...
    def test_add_disjoint(self):
        ...
        self.assertEqual(
            s + t, SortedFrozenSet([1, 2, 3, 4, 5, 6])
        )
    ...
    # # Will only work if we inherit the Sequence base class
    # def test_protocol(self):
...
# There is no need to implement this because Python will
# negate the __eq__() implementation for Inequality. If 
# there is a need to implement this for performance reasons,
# implement __ne__().
